...
// Time rule validation (for no-tag times)
export function isValidTimeRule(rule) {
  if (!rule || typeof rule !== 'object') return false;

  const { days, startTime, endTime } = rule;
  ...
  if (typeof startTime !== 'string' || !timeRegex.test(startTime)) return false;
  if (typeof endTime !== 'string' || !timeRegex.test(endTime)) return false;

  return true;
}

// Sanitize settings (apply defaults and clamp values)
export function sanitizeGameSettings(settings) {
  const limits = GAME_SETTINGS_LIMITS;
  ...
  if (sanitized.duration !== undefined && sanitized.duration !== null) {
    sanitized.duration = clamp(
      Number(sanitized.duration) || limits.duration.default,
      limits.duration.min,
      limits.duration.max
    );
  }
...
// UUID validation (for user/game IDs)
...
// Simple ID validation (for non-UUID IDs)
...
// Avatar validation (emoji or URL)
