...
function Navigation() {
  ...
  return (
    <nav
      ...
      className="fixed bottom-0 left-0 right-0 backdrop-blur-lg z-40"
      style={{
        ...
        {tabs.map(({ path, icon: Icon, label }) => {
          ...
          return (
            <button
              ...
              className="relative flex flex-col items-center justify-center min-w-[64px] min-h-[56px] rounded-2xl transition-all active:scale-95 focus:outline-none"
              style={{
                color: isActive ? 'var(--accent-primary)' : 'var(--text-muted)',
                backgroundColor: isActive ? 'var(--glow-primary)' : 'transparent'
              }}
            >
              <Icon
